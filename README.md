...
The application keeps games, positions, moves, evaluations, and analysis in the browser and never sends them to an external API. Surprise strategies use the fixed catalog and bounded engine-safety fallback; no learning or telemetry service is included. Threaded WASM is selected when cross-origin isolation is available; otherwise the app falls back to the single-thread build.
